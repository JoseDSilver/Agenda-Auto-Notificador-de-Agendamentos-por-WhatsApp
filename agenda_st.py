'''Código para mostrar o funcionamento do código sem o timer de uma hora, Favor não alterar
   agenda sem timer - st'''
...
def obter_info(navegador, agendamentos, hoje, navegador_whats):
...
def inicializar_whats():
    ...
    navegador_whats.maximize_window()
    # Espera o WhatsApp Web carregar
    try:
        WebDriverWait(navegador_whats, 120).until(EC.presence_of_element_located((By.ID, "side")))
    except:
        navegador.quit()
        navegador_whats.quit()
        exit
...
obter_info(navegador, agendamentos, hoje, navegador_whats)
